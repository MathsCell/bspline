...
#' @param regular_grid Logical scalar, should regular knot grid be used when xki=NULL (FALSE by default)
...
#' @export
smbsp=function(x, y, n=3L, xki=NULL, nki=1L, lieq=NULL, monotone=0, positive=0, mat=NULL, estSD=FALSE, tol=1.e-10, regular_grid=FALSE) {
    y=as.matrix(y)
    nx=length(x)
    nc=ncol(y)
    stopifnot(nx == nrow(y))
    ...
    if (is.null(xki)) {
        # build default xki
        if (regular_grid) {
            xki=seq(x[1L], x[nx], length.out=nki+2L)[-c(1L, nki+2L)]
        } else {
    ...
    if (!is.null(lieq)) {
        ...
        if (length(nm_lieq <- names(lieq)) == 0L)
            nm_lieq=paste0("eq_", seq_along(lieq))
        
        liece=mapply(nm_lieq, lieq, SIMPLIFY=FALSE, FUN=function(nm, meq) {
            if (is.null(meq) || length(meq) == 0L) {
                NULL
            } else {
                if (any(ibad <- meq[,1L] < rax[1L] | meq[,1L] > rax[2L]))
                    stop("The following arguments for equality constraint '", nm,
                        "' are out of x range [", rax[1L], "; ", rax[2L], "]:\n\t",
                        paste0("x=", meq[ibad,1L], collapse="\n\t"), call.=FALSE)
