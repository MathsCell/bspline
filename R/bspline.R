...
#' @alias smbsp
#' @export
smbsp=function(x, y, n=3L, xki=NULL, nki=1L, lieq=NULL, monotone=0, mat=NULL) {
    y=as.matrix(y)
    nx=length(x)
    nc=ncol(y)
    stopifnot(nx == nrow(y))
...
fitsmbsp=function(x, y, n=3L, xki=NULL, nki=1L, lieq=NULL, monotone=0, control=list()) {
    ...
    fresid=function(xki, cjac) {
        if (cjac) {
            ...
            f=smbsp(x, y, n=n, xki=xki, nki=0, lieq=lieq, monotone=monotone, mat=li$mat)
            res=f(x)-y
            jac=jacw(li$jac, bsppar(f)$qw)[,,(n+1)+seq_len(nki), drop=FALSE]
            dim(jac)=c(np*ncol(y), nki)
            list(res=res, jacobian=jac)
        } else {
            list(res=smbsp(x, y, n=n, xki=xki, nki=0, lieq=lieq, monotone=monotone)(x)-y)
        }
    }
...
iknots=function(x, y, nki=1L, n=3L) {
    ...
    dubx=duplicated(x)
    if (any(dubx)) {
        xy=as.matrix(aggregate(xy, by=list(x), mean)[,2L:3L])
    }
    dxyn=diffn(xy, n)
    dxy=diffn(dxyn, 1L)
    dy=dxy[,-1L, drop=FALSE]
    xtv=dxyn[,1L]
    tv=rbind(0, arrApply::arrApply(abs(dy)*diff(xy[,1L], lag=n+1), 1, "cumsum"))
